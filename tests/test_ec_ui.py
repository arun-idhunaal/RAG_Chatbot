"""EC-UI — Streamlit UI rendering contracts (Phase 5 / FR-12)."""
...
from app.ui_copy import DISCLAIMER, EXAMPLE_QUESTIONS, WELCOME_MESSAGE


def test_ec_ui_01_welcome_and_three_examples():
    assert "facts only" in WELCOME_MESSAGE.lower() or "facts-only" in WELCOME_MESSAGE.lower()
    assert "investment advice" in WELCOME_MESSAGE.lower()
    assert len(EXAMPLE_QUESTIONS) == 3
    # Cover scheme fact, general fact, and comparison (Architecture §7).
...
def test_ec_ui_03_citation_links_and_inline_date():
    ...
    view = build_assistant_view(result)
    assert "Last updated from sources: 2026-08-12" in view.fact_text
    assert view.citations
    md = citations_markdown(view.citations)
...
def test_ec_ui_04_mixed_fact_and_refusal_visually_split():
    ...
    view = build_assistant_view(result)
    assert view.is_mixed is True
    assert view.refusal_text
    assert "0.5%" in view.fact_text
    assert "investment advice" in (view.refusal_text or "").lower()
...
def test_ec_ui_06_examples_are_real_pipeline_queries():
    """Example buttons must send real questions — not hardcoded fake answers."""
    assert len(EXAMPLE_QUESTIONS) == 3
    for q in EXAMPLE_QUESTIONS:
        assert isinstance(q, str) and len(q) > 10
        assert "?" in q
    # Distinct intents covered by copy (scheme / general / comparison).
